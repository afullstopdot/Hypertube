...
class AuthController extends Controller
{
  public function getOauthGoogleSignIn($request, $response)
  {
    ...
    if (isset($code)) {
      ...
      '&code=' . $code .
      '&redirect_uri=' . 'http://e5r1p4:8080/hypertube/home/oauth/google/signin';
  ...
  public function getOauthGoogleSignUp($request, $response)
  {
    ...
    if (isset($code)) {
      ...
      '&client_secret=IDfZXm5WBAKBXREhX25VCkZh' .
      '&code=' . $code .
      '&redirect_uri=' . 'http://e5r1p4:8080/hypertube/home/oauth/google/signup';
      // send post using curl
      $ch = curl_init($url);
      curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
      curl_setopt($ch, CURLOPT_POST, 1);
  ...
  public function getOauth42SignIn($request, $response)
  {
    ...
    if (isset($code) && isset($state))
    {
      ...
      if ($state === $_SESSION['state'])
      {
        ...
        '&code=' . $code .
        '&redirect_uri=' . 'http://e5r1p4:8080/hypertube/home/oauth/42/signin' .
  ...
  public function getOauth42SignUp($request, $response)
  {
    ...
    if (isset($code) && isset($state))
    {
      ...
      if ($state === $_SESSION['state'])
      {
        ...
        '&client_secret=81ace918b83a8e304ddbd65083a4f5d68ce798c80f83551bc0850bdf58b90721' .
        '&code=' . $code .
        '&redirect_uri=' . 'http://e5r1p4:8080/hypertube/home/oauth/42/signup' .
        '&state=' . $_SESSION['state'];
        // send post using curl
        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_POST, 1);
